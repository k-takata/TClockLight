...
void LoadDrawingSetting(HWND hwnd)
{
	...
	if(!(g_winver&WINXP) && !m_fillbackcolor)
	{
	...
	LoadAnalogClockSetting();	// ANALOGCLOCK
}
...
void CreateClockDC(HWND hwnd)
{
	...
	InitAnalogClock(hwnd, hdc);	// ANALOGCLOCK
	...
	const wchar_t* str, int *wout, int *hout)
{
	...
	while(*p)
	{
		...
		if(!GetTextExtentPoint32W(hdc, sp, ep - sp, &sz))
...
void DrawClock(HWND hwnd, HDC hdc, const SYSTEMTIME* pt)
{
	...
	while(*p)
	{
		...
		case 1:		// shadow
			...
			TextOutW(m_hdcClock,
				x + m_nShadowRange, y + m_nShadowRange, sp, ep - sp);
			SetTextColor(m_hdcClock, colText);
			break;
		case 2:		// border
			SetTextColor(m_hdcClock, colShadow);
			TextOutW(m_hdcClock, x - 1, y - 1, sp, ep - sp);
			TextOutW(m_hdcClock, x    , y - 1, sp, ep - sp);
			TextOutW(m_hdcClock, x + 1, y - 1, sp, ep - sp);
			TextOutW(m_hdcClock, x - 1, y    , sp, ep - sp);
			TextOutW(m_hdcClock, x + 1, y    , sp, ep - sp);
			TextOutW(m_hdcClock, x - 1, y + 1, sp, ep - sp);
			TextOutW(m_hdcClock, x    , y + 1, sp, ep - sp);
			TextOutW(m_hdcClock, x + 1, y + 1, sp, ep - sp);
...
BOOL InitAnalogClock(HWND hwnd, HDC hdc)
{
	...
	if (m_fname[0]) {
		...
		if (m_hbmAclk) {
			...
			if (GetBmpSize(m_hbmAclk, &sz)) // dllutl.c
				size = (sz.cx < sz.cy) ? sz.cx : sz.cy;
			else
				size = -1;
			if (m_nAClockSize == 0)
