...
{
	m_sec = GetMyRegLong(NULL, "IntervalSysInfo", 4);
	if (m_sec < 1 || 60 < m_sec) m_sec = 4;
...
void MemoryHandler(FORMATHANDLERSTRUCT* pstruc)
{
	...
	else if (*(pstruc->sp + 1) == 'A')
	{
		...
		{
			m = ms.dwAvailPageFile;
			t = ms.dwTotalPageFile;
		}
		else if (*(pstruc->sp + 2) == 'V')
		{
			m = ms.dwAvailVirtual;
	...
	else if (*(pstruc->sp + 1) == 'U')
	{
		if (*(pstruc->sp + 2) == 'P')
		{
			m = ms.dwTotalPhys - ms.dwAvailPhys;
			t = ms.dwTotalPhys;
		}
		else if (*(pstruc->sp + 2) == 'F')
		{
			m = ms.dwTotalPageFile - ms.dwAvailPageFile;
			t = ms.dwTotalPageFile;
		}
		else if (*(pstruc->sp + 2) == 'V')
		{
			m = ms.dwTotalVirtual - ms.dwAvailVirtual;
			t = ms.dwTotalVirtual;
		}
		else
			bValid = FALSE;
	...
	if (bValid) {
		...
		pstruc->sp += 4;
		GetNumFormat(&pstruc->sp, 'x', ',', &len, &slen, &bComma);
		SetNumFormat(&pstruc->dp, (unsigned)m, len, slen, bComma);
	} else
		*pstruc->dp++ = *pstruc->sp++;
}
